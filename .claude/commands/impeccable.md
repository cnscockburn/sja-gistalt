...
```bash
node C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/scripts/load-context.mjs
```
...
Load the matching reference file: `C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/reference/brand.md` or `C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/reference/product.md`. The shared design laws below apply to both.
...
- **Second-order:** if someone could guess the aesthetic family from category-plus-anti-references ("AI workflow tool that's not SaaS-cream → editorial-typographic", "fintech that's not navy-and-gold → terminal-native dark mode"), it's the trap one tier deeper. The first reflex was avoided; the second wasn't. Rework until both answers are not obvious. The brand register's reflex-reject aesthetic lanes in `C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/reference/brand.md` list catches the currently-saturated families.
...
2. **First word matches a command**: read its reference file (absolute path in the table above) and follow its instructions. Everything after the command name is the target.
3. **First word doesn't match**: general design invocation. Apply the setup steps, shared design laws, and the loaded register reference, using the full argument as context.

Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke `/impeccable`.

If the first word is `craft`, setup still runs first, but `C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/reference/craft.md` owns the rest of the flow. If setup invokes `teach` as a blocker, finish teach, refresh context, then resume the original command and target.
...
```bash
node C:/Users/cnsco/Git/pdf-tools/.agents/skills/impeccable/scripts/pin.mjs <pin|unpin> <command>
```
